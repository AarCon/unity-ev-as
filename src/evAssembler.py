...
class MacroAssembler:
    ...
    def processTextMacro(self, cmdType, macro, commands, strTbl, tags):
        ...
        macroCommands.append(
            EvCmd(cmdType, evCmdArgs, macro.line, macro.column, self.fileName)
        )
...
class evAssembler(evListener):
    ...
    def enterInstruction(self, ctx: evParser.InstructionContext):
        ...
        if self.macro.isValid():
            self.currCmdIdx += self.macroAssembler.process(
                self.macro, self.scripts[self.currentLabel], self.strTbl, self.tags
            )
            self.macro = EvMacro(
                EvMacroType.Invalid, [], ctx.start.line, ctx.start.column
            )
